<script lang="ts">
  import { defense_masters, defense_masters_meta } from "$lib/data";
  ...
  const endpoint = 'https://sqlsamurai.fike.io/api/defense_masters';
  let entity = 'Defense Masters';
  let array = defense_masters;
  let currentId = 0;
  let lowerIndex = 0;
  let upperIndex = 0;
...
<div class="container max-w-screen-lg bg-neutral text-neutral-content rounded-lg ml-16 my-16 shadow-xl">
  ...
  <div class="p-8 flex flex-col gap-8">
    <div class="flex flex-row justify-between gap-4">
        ...
        <div slot="form">
          {#each Object.entries(array[currentId]) as [name, value], i}
            <div class="form-control w-full max-w-xs">
              <label for="id" class="label"><span class="label-text font-light">{defense_masters_meta[i]}</span></label>
      ...
      <DeleteItemMenu {endpoint} disabled={selectableArray.filter(i => i.checked).length === 0} queued={selectableArray.filter(a => a.checked).map(a => `${a.obj.master_name} (${a.obj.master_id})`)} queuedIds={selectableArray.filter(a => a.checked).map(a => a.obj.master_id)}>
        <span slot="entity">{entity}</span>
    ...
    <table class="table">
      <thead>
        <tr>
          <th></th>
          {#each defense_masters_meta as meta}
            <th>{meta}</th>
          {/each}
          <th></th>
        </tr>
