<script lang="ts">
  ...
  let tableName = 'Battleground Towns';
  let arrayTowns: any[] = [];
  let arrayClans: any[] = [];
  let lowerIndex = 0;
  let upperIndex = 0;
...
<div class="container max-w-screen-lg bg-neutral text-neutral-content rounded-lg ml-16 my-16 shadow-xl">
  <div class="p-8 flex flex-col gap-8">
    <div class="flex flex-row justify-between gap-4">
      ...
      <DeleteItemMenu {endpoint} disabled={selectableArray.filter(i => i.checked).length === 0} queued={selectableArray.filter(a => a.checked).map(a => `${a.obj.town_name} (${a.obj.town_id})`)} queuedIds={selectableArray.filter(a => a.checked).map(a => a.obj.town_id)}>
        <span slot="entity">{tableName}</span>
      ...
      <Paginate totalRows={arrayTowns.length} bind:lowerIndex bind:upperIndex />
    </div>
    <table class="table">
      <thead>
        <tr>
